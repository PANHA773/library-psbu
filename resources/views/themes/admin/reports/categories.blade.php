    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>{{ __('admin.category_report') }}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="#">{{ __('admin.home') }}</a></li>
                            <li class="breadcrumb-item active">{{ __('admin.category_report') }}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
        ...
        <section class="content">
            <div class="container-fluid">
                <form method="POST" action="{{ admin_url('reports/categories') }}">
                    @csrf
                    @method('POST')
                                ...
                                <tbody>
                                    @foreach ($categories as $book)
                                        <tr class="text-center">
                                            <td><img width="50px"
                                                    src="{{ $book->image ? asset('uploads/category/' . $book->image) : asset('images/no_image.png') }}">
                                            </td>
                                            <td>{{ $book->code }}</td>
                                            <td>{{ $book->name }}</td>
                                            <td>{{ $book->slug }}</td>
                                            <td>{{ $book->parent_name }}</td>
                                            <td>
                                                {{ $book->created_at }}
