...
<div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{__('admin.edit_book_borrowing')}}</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">{{__('admin.home')}}</a></li>
              <li class="breadcrumb-item active">{{__('admin.edit_book_borrowing')}}</li>
            </ol>
          </div>
        </div>
      </div>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card">
              ...
              <form id="quickForm" action="{{ admin_url('borrowers/'. $id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row">
                      <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="date" name="date" class="form-control" value="{{ $borrow->start_date }}" id="date">
                        </div>
                      </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="reference_no">{{__('admin.reference_no')}}</label>
                                <input type="text" name="reference_no" class="form-control" id="reference_no" value="{{$borrow->reference_no}}"  placeholder="Enter reference_no">
                            </div>
                        </div>
                        <div class="col-md-4">
                          <div class="form-group">
                              ...
                              <select class="form-control select2" style="width: 100%;" name="student_id">
                                <option value="" disabled>{{__('admin.select_student')}}</option>
                                @foreach($students as $student)
                                <option value="{{ $student->id }}" {{$student->id == $borrow->student_id ? 'selected' : ''; }}>{{ $student->first_name.' '. $student->last_name; }}</option>
                                @endforeach
                              </select>
                          </div>
                        </div>
                        ...
                        <div class="col-md-12 mt-4">
                            <div class="card card-outline card-info">
                              <div class="card-header">
                               ...
                               <textarea id="summernote" name="description">{{decode_html($borrow->note)}}</textarea>
                            </div>
                          </div>
                    </div>
                </div>
...
<script>     
    ...
    if('<?= $book_borr ?>') {
      localStorage.setItem('britems', '<?= $book_borr ?>');
    }
